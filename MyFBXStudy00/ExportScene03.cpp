...
const char* ExportScene03::gDiffuseElementName = "DiffuseUV";
const char* ExportScene03::gAmbientElementName = "AmbientUV";
const char* ExportScene03::gEmissiveElementName = "EmissiveUV";

int ExportScene03::DoMain(int argc, char** argv)
{
	...
	lSdkManager->RegisterFbxClass("MyFbxObject", FBX_TYPE(MyFbxObject), FBX_TYPE(FbxObject), "MyFbxObjectType",
	                              "MyFbxObjectSubType");
	...
	if (!lSampleFileName)
		lSampleFileName = gExportVertexCacheMCFormat
			                  ? (char*)SAMPLE_FILENAME_MC
			                  : (char*)SAMPLE_FILENAME_PC2;
...
bool ExportScene03::CreateScene(FbxScene* pScene, char* pSampleFileName)
{
	...
	if (lMyProperty.IsValid())
	{
		...
		if (lMyFCurve)
		{
			...
			key.Set(FBXSDK_TIME_ZERO, -100);
			lMyFCurve->KeyAdd(key.GetTime(), key);
			key.Set(FbxTime(100), 0);
			lMyFCurve->KeyAdd(key.GetTime(), key);
			key.Set(FbxTime(200), 100);
			lMyFCurve->KeyAdd(key.GetTime(), key);
		}
	}
...
FbxNode* ExportScene03::CreateCubeWithTexture(FbxScene* pScene, const char* pName)
{
	int i, j;
...
FbxNode* ExportScene03::CreatePyramidWithMaterials(FbxScene* pScene, const char* pName)
{
	int i, j;
	FbxMesh* lMesh = FbxMesh::Create(pScene, pName);
...
FbxNode* ExportScene03::CreateTriangle(FbxScene* pScene, const char* pName)
{
	FbxMesh* lMesh = FbxMesh::Create(pScene, pName);
...
FbxNode* ExportScene03::CreateCubeWithMaterialAndMyKFbxMesh(FbxScene* pScene, const char* pName)
{
	int i, j;
	...
	FbxGeometryElementUserData* lFbxGeometryElementUserData = FbxGeometryElementUserData::Create(
		lMyKFbxMesh, "My Geometry Element", PID_MY_GEOMETRY_LEMENT, lArrayType, lArrayNames);
...
void ExportScene03::CreateTexture(FbxScene* pScene, FbxMesh* pMesh)
{
	...
	if (lNode)
	{
		...
		if (lMaterial == NULL)
		{
			...
			FbxLayerElementMaterial* lLayerElementMaterial = FbxLayerElementMaterial::Create(
				pMesh, lMaterialName.Buffer());
...
void ExportScene03::CreateMaterials(FbxScene* pScene, FbxMesh* pMesh)
{
	int i;
...
void ExportScene03::CreateMaterialsWithMyKFbxMesh(FbxScene* pScene, MyKFbxMesh* pMyKFbxMesh)
{
	int i;
	for (i = 0; i != 6; ++i)
	{
...
void ExportScene03::MapShapeOnPyramid(FbxScene* pScene, FbxNode* pPyramid)
{
...
void ExportScene03::MapVertexCacheOnTriangle(FbxScene* pScene, FbxNode* pTriangle, char* pSampleFileName)
{
	...
	FbxString lRelativePCFileName = FbxPathUtils::GetRelativeFilePath(
		FbxPathUtils::GetFolderName(lFBXAbsolutePath) + "/", lAbsolutePCFileName);
	...
	if (gExportVertexCacheMCFormat && gCacheType
...
void ExportScene03::SetCubeDefaultPosition(FbxNode* pCube)
{
...
void ExportScene03::SetPyramidDefaultPosition(FbxNode* pPyramid)
{
...
void ExportScene03::SetTriangleDefaultPosition(FbxNode* pTriangle)
{
...
void ExportScene03::SetMyKFbxMeshCubeDefaultPosition(FbxNode* pMyKFbxCube)
{
...
void ExportScene03::Animate(FbxNode* pNode, FbxAnimLayer* pAnimLayer)
{
...
void ExportScene03::AnimateVertexCacheOnTriangleDoubleVertex(FbxNode* pTriangle, double pFrameRate)
{
	//
	// Move the vertices from their original position to the center.
	//
	FbxVertexCacheDeformer* lDeformer = static_cast<FbxVertexCacheDeformer*>(pTriangle->GetGeometry()->GetDeformer(
		0, FbxDeformer::eVertexCache));
	...
	if (gExportVertexCacheMCFormat)
	{
		...
		lRet = lCache->OpenFileForWrite(FbxCache::eMCOneFile, pFrameRate, pTriangle->GetName(), FbxCache::eMCX,
		                                FbxCache::eDoubleVectorArray, "positions", &lStatus);
		if (lRet)
		{
			FbxString channelName(pTriangle->GetName());
			channelName += "_normals";
			lRet = lCache->AddChannel(channelName, FbxCache::eFloatVectorArray, "normals", lNormalChannelIndex,
			                          &lStatus);
		}
	}
	else
...
void ExportScene03::AnimateVertexCacheOnTriangleInt32(FbxNode* pTriangle, double pFrameRate)
{
	//
	// Move the vertices from their original position to the center.
	//
	...
	if (gExportVertexCacheMCFormat)
	{
		...
		// FbxCache::eMCX with FBXCache::eMCC
		lRet = lCache->OpenFileForWrite(FbxCache::eMCOneFile, pFrameRate, pTriangle->GetName(), FbxCache::eMCX,
		                                FbxCache::eInt32Array, "positions", &lStatus);
	}
...
void ExportScene03::AnimateVertexCacheOnTriangleFloat(FbxNode* pTriangle, double pFrameRate)
{
	//
	// Move the vertices from their original position to the center.
	//
	...
	if (gExportVertexCacheMCFormat)
	{
		...
		// FbxCache::eMCX with FBXCache::eMCC
		lRet = lCache->OpenFileForWrite(FbxCache::eMCOneFile, pFrameRate, pTriangle->GetName(), FbxCache::eMCX,
		                                FbxCache::eFloatVectorArray, "positions", &lStatus);
		if (lRet)
		{
			FbxString channelName(pTriangle->GetName());
			channelName += "_normals";
...
#define CmpFlt(a,b)    (AbsFlt((a)-(b)) > 1e-5)
	...
	while (lCurrentTime2 <= lStopTime && passTest)
	{
		...
		if (gExportVertexCacheMCFormat)
		{
			...
			if ((CmpFlt(lVertices[0][0], -50.0f * lScaleFactor) || CmpFlt(lVertices[0][1], 0.0f) || CmpFlt(
					lVertices[0][2], 50.0f * lScaleFactor)) ||
				(CmpFlt(lVertices[1][0], 50.0f * lScaleFactor) || CmpFlt(lVertices[1][1], 0.0f) || CmpFlt(
					lVertices[1][2], 50.0f * lScaleFactor)) ||
				(CmpFlt(lVertices[2][0], 0.0f * lScaleFactor) || CmpFlt(lVertices[2][1], 50.0f * lScaleFactor) ||
					CmpFlt(lVertices[2][2], -50.0f * lScaleFactor)))
			{
